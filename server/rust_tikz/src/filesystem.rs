...
impl VirtualFileSystem {
    ...
    pub fn get_file_pointer_by_index(
        &self,
        fd: i32,
    ) -> Option<&FilePointer> {
        if fd < 0 {
    ...
    pub fn write_to_file_by_index(&mut self, fd: i32, data: &[u8]) {
        ...
        if let Some(fp) = self.fd_to_file_pointer.get_mut(fd as usize)
        {
            let buffer = match fp.file {
                ...
                FileType::Named(ref name) => {
                    self.new_files.insert(name.clone());
    ...
    pub fn read_from_file_by_index(
        ...
        if let Some(fp) = self.fd_to_file_pointer.get_mut(fd as usize)
        {
            ...
            if fp.byte_seek_position >= buffer.len() {
                return Vec::new();
            }

            let start = fp.byte_seek_position;
            let end =
                min(fp.byte_seek_position + length, buffer.len());
    ...
    pub fn file_pointer_at_eof(&self, fp: &FilePointer) -> bool {
        match &fp.file {
            ...
            FileType::Named(name) => {
                ...
                fp.byte_seek_position >= buffer.len()
                    || fp.text_seek_position >= buffer.len()
            }
        }
    }
    ...
    pub fn file_pointer_at_eoln(&self, fp: &FilePointer) -> bool {
        self.file_pointer_at_eof(fp)
            || match &fp.file {
                FileType::Stdin => {
                    self.stdin.get(fp.text_seek_position)
                        == Some(&b'\n')
                }
                FileType::Stdout => false,
                FileType::Named(name) => {
                    let buffer = self.data.get(name).unwrap();
                    buffer.get(max(fp.text_seek_position, 0))
                        == Some(&b'\n')
                }
            }
    }
    ...
    pub fn set_file_contents(
        ...
        match file {
            ...
            FileType::Named(name) => {
                self.new_files.insert(name.to_string());
