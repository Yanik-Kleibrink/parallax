...
impl TexJaxImports {
    pub(crate) fn new(store: &mut Store<VirtualFileSystem>) -> Self {
        Self {
            ...
            eof: Func::wrap(
                &mut *store,
                |mut caller: Caller<_>, fd: i32| -> i32 {
                    let vfs: &mut VirtualFileSystem =
                        caller.data_mut();
                    let fp =
                        vfs.get_file_pointer_by_index(fd).unwrap();
                    //vfs.file_pointer_at_eof(fp) as i32
                    // When comparing with tikzjax, it seems that eof is only true if the program tried to read a bit past the end of the file.
                    // When e-tex reads a font file it stops before the end as the size of the font file is specifically stored in the file.
                    // As e-tex still reads the entire file the position pointer is eventually equal to the length of the file. This yields an eof using
                    // the old logic of rust-tikz but not using tikzjax.
                    //
                    // Note that it is possible to experiment with tikzjax using the WebPack (inspect -> debug - webpack) of https://tikzjax.com/. There it is rather straightforward to set breakpoints at meaningful positions.
                    0
                },
            ),
            eoln: Func::wrap(
            ...
            get_current_day: Func::wrap(&mut *store, || -> i32 { 1 }),
            get_current_minutes: Func::wrap(
                &mut *store,
                || -> i32 { 0 },
            ),
            ...
            input_ln: Func::wrap(
                ...
                |mut caller: Caller<_>,
                 ...
                 -> i32 {
                    ...
                    let get_first =
                        |caller: &Caller<VirtualFileSystem>| {
                            let first = u8_to_u32(&read_memory(
                                &mem,
                                caller,
                                first_pointer as usize,
                                4,
                            ));
                            first
                        };
                    // Set the u32 stored in the `last_pointer` memory location.
                    let set_last = |last: u32,
                                    caller: &mut Caller<
                        VirtualFileSystem,
                    >| {
                        mem.write(
                            caller,
                            last_pointer as usize,
                            &last.to_ne_bytes(),
                        )
                        .expect("Failed to write to memory");
                    };
                    ...
                    if let Some(mut input_line) =
                        vfs.read_line_by_index(fd)
                    {
                        ...
                        if input_line.len() > 0 {
                            ...
                            mem.write(
                                &mut caller,
                                (buf_pointer + _first as u32)
                                    as usize,
                                &input_line,
                            )
                            .expect("Failed to write to memory");
                            set_last(
                                _first + input_line.len() as u32,
                                &mut caller,
                            );
                        }
            ...
            print_char: Func::wrap(
                &mut *store,
                |mut caller: Caller<_>, fd: i32, char: i32| {
                    let vfs: &mut VirtualFileSystem =
                        caller.data_mut();
                    vfs.write_to_file_by_index(fd, &[char as u8]);
                },
            ),
            print_integer: Func::wrap(
                &mut *store,
                |mut caller: Caller<_>, fd: i32, num: i32| {
                    let vfs: &mut VirtualFileSystem =
                        caller.data_mut();
                    vfs.write_to_file_by_index(
                        fd,
                        num.to_string().as_bytes(),
                    );
                },
            ),
            print_newline: Func::wrap(
                &mut *store,
                |mut caller: Caller<_>, fd: i32| {
                    let vfs: &mut VirtualFileSystem =
                        caller.data_mut();
